...
def generate_filename():
    now = datetime.now()
    return now.strftime("%Y_%m_%d_%H_%M_%S") + ".json"
...
def main():
    ...
    with requests.Session() as session:
        ...
        all_data = {"lastUpdated": int(time.time()), "vehicles": []}
        ...
        for vehicle in vehicles:
            ...
            trip_details = (
                fetch_trip_details(session, gtfs_id, service_day) if gtfs_id else {}
            )
            ...
            lat = vehicle.get("lat")
            lon = vehicle.get("lon")
            lastUpdate = vehicle.get("lastUpdated")
            ...
            route_short_name = trip_details.get("route").get(
                "shortName"
            )  # Useless garbage
        ...
        filename = generate_filename() # Needed for cloudflare caching
        with open(f"/web/{filename}", "w", encoding="utf-8") as f:
            json.dump(all_data, f, separators=(",", ":"), ensure_ascii=False)
        with open("/web/pointer.txt", "w") as f:
            f.write(filename)
